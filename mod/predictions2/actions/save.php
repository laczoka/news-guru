...
if (!empty($option1) && !empty($value1)) {
	$options[]= array('option' => $option1, 'value' => $value1);
}

if (!empty($option2) && !empty($value2)) {
    $options[]= array('option' => $option2, 'value' => $value2);
}

global $CONFIG;
$prediction_backend = $CONFIG->predictions['backend'];

$prediction_backend->savemarket(
     $title,
     $body, 
     get_loggedin_userid(), 
     $tags, 
